...
public class List {
    ...
    */
     ...
     */
    public void resize() {
        newlist = Arrays.copyOf(newlist, 2 * newlist.length);
    }
    /**
     * Adds all items at once.
     *
     * @param      item  string array
     */
    public void addAll(final String[] item) {
        for (int i = 1; i < item.length; i++) {
            add(Integer.parseInt(item[i]));
        }
    }
    /**
     ...
     * @param      item   int type
     */
    public void addat(final int index, final int item) {
        if (size == newlist.length) {
            resize();
        }
        if (index < size) {
    ...
    /**
     * { count function to calculate appearance}.
     *
     * @param      item  The item
     *
    ...
    public static void main(final String[] args) {
        ...
        while (stdin.hasNext()) {
            ...
            case "print":
                // print the list
                // (implement toString for this to work)
            ...
            case "remove":
                ...
                break;
            case "indexOf":
                System.out.println(l.indexOf(Integer.parseInt(
                                                 tokens[1])));
                break;
            case "get":
                if (l.get(Integer.parseInt(tokens[1])) != -1) {
                    System.out.println(
                        l.get(Integer.parseInt(
                                  tokens[1])));
                }
                break;
            case "contains":
                System.out.println(
                    l.contains(Integer.parseInt(
                                   tokens[1])));
                break;
            case "addAll":
                l.addAll(tokens);
                break;
            case "addat":
                l.addat(Integer.parseInt(tokens[1]),
